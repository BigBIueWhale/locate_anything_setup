...
The test_data/ mount carries the drone JPEGs committed to the repo
(used by the worker's boot calibration) and the synthetic smoke-test
JPEG generated by 01_download_weights.sh (used by scripts/04 and 05).
Nothing here is baked into the image because the image is built
before 01_download_weights.sh runs (so test_data/ is empty at build
time) and the drone JPEGs are kept as bind-mounted assets rather
than image layers.

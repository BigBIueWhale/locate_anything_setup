...
* **Detecting a drone that subtends ≥ 60 px in the source image,
  ...
  camera in daylight. Send `request:{task:"point",phrase:"drone in
  the sky"}` in `slow` mode (a `point` task → a `points` reply).

* **Verifying / classifying ROIs already cropped by another
  detector.** When the drone occupies a significant fraction of
  the crop, LocateAnything's open-vocab strength kicks in. Send
  `request:{task:"phrase_single",phrase:"a quadcopter drone"}` on
  the ROI (→ a `boxes` reply).
...
* Per tile, JPEG-encode, send a Frame with `generation_mode = "slow"`
  and the typed request
  `request:{task:"phrase_multi",phrase:"a small drone in the sky"}`
  (the server compiles it to the trained multi-instance grounding
  prompt; see [`docs/CLIENT_PROTOCOL.md`](./CLIENT_PROTOCOL.md)).
* For each reply, match `type=="boxes"` and read `boxes[].bbox_px`;
  translate back into source-image coords using the tile's offset. A
  tile with no drone comes back as `type=="abstained"` (skip it).

...
```
        ┌─────────────────── Docker container ───────────────────┐
        │                                                        │
client ─┼─→ TCP :8000 ─→ [ Rust la_server ] ──Unix domain socket /tmp/la.sock──→│ [ Python la_worker ] ─→ GPU
...
* `/opt/locate_anything/test_data/drone_sirius.jpg` (and `_byrobot`,
  `_r18`, `cat_negative`) — drone JPEGs committed to the repo; the
  worker's boot calibration defaults to `drone_sirius.jpg` so the
  published `median_fps` is workload-representative.
* `/opt/locate_anything/test_data/calibration.jpg` — synthetic
  1024×768 four-polygon image generated by
  `scripts/01_download_weights.sh`; used by
  `scripts/04_smoke_test.sh` and `scripts/05_concurrency_smoke.sh`
  (the "we know what categories to expect" property is load-bearing
  there). Not used by boot calibration.
